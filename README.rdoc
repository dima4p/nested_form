...
NB! The rest of this paragraph is valid only for jQuery so far.

You can wrap +link_to_add+ into one or more tags. In that case the outmost tag should 
have class 'add_link'
  ...
  <%= f.fields_for :tasks do |task_form| %>
    ...
    <%= task_form.link_to_remove "Remove this task" %>
  <% end %>
  <p class="add_link"><%= f.link_to_add "Add a task", :tasks %></p>

Additinally, you can put +link_to_add+ to any place inside the form. In that case you should place any element with class 'add_association_link' immediately below
+fields_for+
...
If you want other tag (not <div>) to be used to hold the content of the inserted partial, say, <tr>, use option :enclosing_tag.

  <%= f.fields_for :tasks, :enclosing_tag => 'tr' %>
